...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create a FormData object (Standard format for Google Scripts)
      const data = new FormData();
      data.append('name', formData.name);
      data.append('email', formData.email);
      data.append('company', formData.company);
      data.append('message', formData.message);
      // Add a timestamp for the sheet
      data.append('date', new Date().toISOString());

      // Send to Google Sheets
      // mode: 'no-cors' is CRITICAL here. It allows the browser to send data 
      // to Google without security errors, but it means we won't get a readable response back.
